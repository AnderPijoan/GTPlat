...
PostgresDriver::PostgresDriver(QString url, int port, QString db_name, QString db_user, QString db_pass, QString connection_name) : QObject()
{
    ...
    if( !QSqlDatabase::drivers().contains( psql_driver ) ){
        Debugger::critical( this , Q_FUNC_INFO , QString("Driver %1 is not available. Options are %2").arg( psql_driver ).arg( this->database.drivers().join(',') ) );
    }
    ...
    while( !this->database.open() ){
        ...
        Debugger::critical( this , Q_FUNC_INFO , QString("Unable to connect to DB %1 %2 %3 waiting... ").arg( this->database.hostName() ).arg( this->database.port() ).arg( this->database.databaseName() ) );
    }
}
...
QList<QSqlRecord> PostgresDriver::executeQuery( QString sql ){
    ...
    if( this->database.isOpen() ){
        ...
        if (error.type() != QSqlError::NoError){
            Debugger::warning( this , Q_FUNC_INFO , QString("%1 \n %2").arg( error.text() ).arg( sql ) );
        }
        ...
        Debugger::critical( this , Q_FUNC_INFO , "Could not execute query because database is closed");
    }
