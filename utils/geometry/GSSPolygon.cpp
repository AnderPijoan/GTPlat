...
GSSPolygon::GSSPolygon( geos::geom::Polygon *geom, QObject *parent) : GSSGeometry( geom , parent ){
...
QImage GSSPolygon::toImage(const GSSEnvelope image_bounds, int image_width, int image_height, QColor border_color, int border_width, QColor fill_color, QImage icon) const{
    Q_UNUSED(icon);

    // If bounds are too big, just return coordinate
    if( image_bounds.getArea() > (this->getArea() * 50) ){
...
QJsonObject GSSPolygon::toGeoJSON(QJsonObject properties) const{
