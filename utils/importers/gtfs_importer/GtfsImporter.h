...
public:
    ...
    explicit GtfsImporter(QString files_folder_path);
    ~GtfsImporter();
    ...
    // METHODS
    void createStretchsInEnvironment(  Environment* environment , QDateTime from = QDateTime::currentDateTime() , QDateTime to = QDateTime::currentDateTime().addDays(1) );
    QList<PublicTransportVehicleDriver*> getGTFSDrivers( Environment* environment , QDateTime from = QDateTime::currentDateTime() , QDateTime to = QDateTime::currentDateTime().addDays(1) );
...
private:
    ...
    PublicTransportStop* snapToStopInEnvironment( Environment* environment , GtfsImporter::Stop stop , QString vehicle_type );
};
