...
QJsonObject GSSHeatMap::toJSON(){
    QJsonArray array;
    foreach( GSSCoordinate coor , this->points.keys() ){
        QJsonArray c;
        c.append( coor.getY() );
        c.append( coor.getX() );
...
QImage GSSHeatMap::toImage(const GSSEnvelope image_bounds, int image_width, int image_height) const{
...
void GSSHeatMap::addPoint(const GSSCoordinate &point , double weight ){
    if( point.isValid() ){
