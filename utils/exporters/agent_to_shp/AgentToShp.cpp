#include "AgentToShp.h"

#include <QDir>

QString AgentToSHP::shp_folder_name = "shps";

AgentToSHP::AgentToSHP(QObject *parent) : QObject(parent){

    // Check if common_path folder exists
    if( !QDir( QDir::currentPath() + "/" + AgentToSHP::shp_folder_name ).exists() ){
        Debugger::debug( this , Q_FUNC_INFO , QString("Creating shps folder in %1/%2").arg( QDir::currentPath() ).arg( shp_folder_name ) );
        QDir().mkdir( QDir::currentPath() + "/" + AgentToSHP::shp_folder_name );
    }
...
bool AgentToSHP::exportAgents(QList<Agent *> agents, QString filename, QString geometry_type){
    ...
    this->shp_handle = SHPCreate( (QDir::currentPath() + "/" + AgentToSHP::shp_folder_name + "/" + filename).toStdString().c_str(), g_type );
    this->dbf_handle = DBFCreate( (QDir::currentPath() + "/" + AgentToSHP::shp_folder_name + "/" + filename).toStdString().c_str() );
    ...
    for(int a = 0; a < agents.size(); a++){
        ...
        QList<GSSCoordinate> coordinates = agents.at(a)->getGeometry()->getCoordinates();
        ...
        QJsonObject properties = agents.at(a)->toJSON()["properties"].toObject();
        foreach( QString key , properties.keys() ){
            ...
            if( pos == -1 ){
                ...
                if( pos > -1 ){
                    ...
                    Debugger::warning( this , Q_FUNC_INFO , QString("Could not create DBF field %1").arg( key ) );
                }
            }
